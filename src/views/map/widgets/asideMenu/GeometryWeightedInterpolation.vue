...
<template>
  <div>
    <!-- <p class="analyseTitle">反距离加权插值</p> -->
...
export default {
  ...
  methods: {
    init(){//初始化
      ...
      this.analyseForm.resultTitle='反距离加权插值'//结果图层名称
    },
    ...
    getAnalyseFormLayer(){//获取选取的分析图层
      ...
      if(layer){
        ...
        this.analyseForm.resultTitle=`${layer.title}反距离加权插值`;
      }
    },
    handleDone(formName) {//分析
      this.$refs[formName].validate((valid) => {
        if (valid && this.analyseForm.radius>0) {
          let layerid=this.analyseForm.layer
